...
/** The local API is HTTPS with a self-signed certificate Riot generates per install, so there is no
 * chain to verify it against. Verification is disabled for this request only — not process-wide via
 * NODE_TLS_REJECT_UNAUTHORIZED — and the connection never leaves the loopback interface. */
