  ...
  return (
    <form onSubmit={submit} className="rounded-xl border border-line bg-surface p-3">
      <div className="mb-2 flex items-baseline justify-between gap-2">
        <h2 className="text-sm font-semibold">Quick issue capture</h2>
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <select value={repo} onChange={(event) => setRepo(event.target.value)} className="rounded border border-line bg-canvas px-2 py-1.5 text-sm" disabled={!repos.length}>
          {repos.map((item) => <option key={item} value={item}>{item}</option>)}
        </select>
        <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Capture an issue…" maxLength={160} className="min-w-0 flex-1 rounded border border-line bg-canvas px-2 py-1.5 text-sm" />
      ...
      {detail && <textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder="Optional context…" rows={3} className="mt-2 w-full rounded border border-line bg-canvas px-2 py-1.5 text-sm" />}
